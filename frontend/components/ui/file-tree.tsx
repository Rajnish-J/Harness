...
function Folder({
  ...
  return (
    ...
    >
      <AccordionPrimitive.Header className="flex">
        <AccordionPrimitive.Trigger
          className="group/tree-folder flex w-full items-center gap-1 rounded px-2 py-0.5 text-left text-xs outline-none hover:bg-accent focus-visible:ring-[3px] focus-visible:ring-ring/50"
          style={{ paddingLeft: depth * 12 + 8 }}
        >
          {/* Rotation keys off the Trigger's own data-state, which Radix
              stamps, rather than a second attribute driven from React state. */}
...
function File({
  ...
  return (
    <button
      ...
      style={{ paddingLeft: depth * 12 + 20 }}
      className={cn(
        "flex w-full items-center gap-1 rounded px-2 py-0.5 text-left text-xs outline-none focus-visible:ring-[3px] focus-visible:ring-ring/50",
        isSelected ? "bg-accent font-medium" : "hover:bg-accent",
        disabled && "cursor-not-allowed opacity-40 hover:bg-transparent",
        className
      )}
