...
| 11 | Data persistence | Done | `frontend/db/schema.ts` + 16 Drizzle migrations (`0000..0015*.sql`), 9 `backend/app/db/*_repo.py` + `pool.py` (psycopg pool) | Drizzle owns schema/migrations; backend queries the same Postgres DB via a hand-written repo layer. → Evidence refreshed in this branch (new `0015_strange_stellaris.sql` adds the `tool_settings` table). |
...
| 17 | Accessibility | Partial | `eslint.config.mjs` (`eslint-config-next/core-web-vitals`+`/typescript`, bundles `jsx-a11y`), ARIA usage across 46 files via radix-ui primitives, `sr-only`/`VisuallyHidden` in 19 | Linted in CI; no axe/a11y test tooling. → Evidence refreshed in this branch (`ToolGroupDialog.tsx` and `ToolsBrowser.tsx` gain `aria-label`s on the new tool switches). |
| 18 | Responsiveness | Partial | Tailwind `sm:`/`md:`/`lg:`/`xl:` classes in 33 files, `frontend/hooks/use-mobile.ts` | Desktop-first coverage, not exhaustively responsive. → Count corrected to 33 (was 34 at `origin/main`): this branch moved `ToolGroupDialog.tsx`'s own `sm:max-w-2xl` into the shared `sheet.tsx` `floating` variant, which carries the breakpoints instead. |
| 19 | User flows | Missing | `backend/tests/*.py` (56 files, unit/component level) | No Playwright/Cypress/E2E, no `e2e/` dir, no frontend test runner at all. → Evidence refreshed in this branch (new `test_cors.py`, the first CORS assertions in the suite); the gap itself — no E2E — is unchanged. |
