...
| 8 | Loading states | Partial | `frontend/components/ui/skeleton.tsx`; `Loader2`/`isLoading`/`isPending` across 11 files | Widely used but not systematic/audited. → Evidence refreshed in this branch (chat's new AttachProposalCard.tsx and OpenAsProjectDialog.tsx add two more). |
...
| 11 | Data persistence | Done | `frontend/db/schema.ts` + 14 Drizzle migrations (`0000..0013*.sql`), 9 `backend/app/db/*_repo.py` + `pool.py` (psycopg pool) | Drizzle owns schema/migrations; backend queries the same Postgres DB via a hand-written repo layer. → Evidence refreshed in this branch (two new migrations: `project_chat_messages` project index, `pinned_at`). |
...
| 17 | Accessibility | Partial | `eslint.config.mjs` (`eslint-config-next/core-web-vitals`+`/typescript`, bundles `jsx-a11y`), ARIA usage across 44 files via radix-ui primitives | Linted in CI; no axe/a11y test tooling, no explicit `sr-only`/`VisuallyHidden`. → Evidence refreshed in this branch (new dropdown/dialog components add `sr-only` labels). |
| 18 | Responsiveness | Partial | Tailwind `sm:`/`md:`/`lg:`/`xl:` classes in 31 files, `frontend/hooks/use-mobile.ts` | Desktop-first coverage, not exhaustively responsive. → Evidence refreshed in this branch (compact IDE-rail variant adds responsive-style breakpoint usage). |
| 19 | User flows | Missing | `backend/tests/*.py` (41 files, unit/component level) | No Playwright/Cypress/E2E, no `e2e/` dir, no frontend test runner at all. → Evidence refreshed in this branch (9 new backend test files); the gap itself — no E2E — is unchanged. |
