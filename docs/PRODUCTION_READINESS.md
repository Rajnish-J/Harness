...
| 19 | User flows | Missing | `backend/tests/*.py` (55 files, unit/component level) | No Playwright/Cypress/E2E, no `e2e/` dir, no frontend test runner at all. → Evidence refreshed in this branch (new `test_tool_settings.py`); the gap itself — no E2E — is unchanged. |
