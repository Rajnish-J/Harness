...
**Merged** 2026-09-11 · created 2026-09-08 · by @Rajnish-J · 46 commits · 97 files, +15226 / −337 · branch still on the remote

**feat(tools): route each turn's toolset and ask before using MCP**

Every turn used to serialize the whole tool list into every request — 43 built-ins plus whatever the attached MCP servers contribute — which on a small-context model crowds out the conversation itself. This branch puts a routing step in front of each turn so a turn only ever sees the tools its message actually needs, adds the escape hatches that make narrowing safe, and gives each conversation its own URL along the way. It builds on main at 4fd5ceb, and carries the fix/chat-delete CORS fix that landed there independently.
...
**Merged** 2026-09-08 · created 2026-09-05 · by @Rajnish-J · 29 commits · 128 files, +10976 / −489 · branch still on the remote

**feat(mcp): remote MCP servers with credential-backed auth, plus the coding toolset**

Connects the harness to remote MCP servers end to end — credential-backed auth, real connection status in the UI, and a GitHub catalog entry — then builds out the coding toolset the agent uses once it is connected.

### `feat/provider-key-text-input` → `main` ([#10](https://github.com/Rajnish-J/Harness/pull/10))

**Merged** 2026-09-05 · created 2026-09-04 · by @Rajnish-J · 20 commits · 99 files, +10965 / −384 · branch still on the remote

**feat(chat): scaffold projects and manage conversations as workspaces**

Adds project scaffolding and a set of chat/project workspace features on top of main, plus a couple of smaller fixes. This is a mixed diff — 54 new files and 44 modified files, not purely additive.
